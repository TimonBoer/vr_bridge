...
class SliderPublisher(Node):
    def __init__(self):
        super().__init__('talker_quaternion_gui')
        self.pub = self.create_publisher(Quaternion, 'orientation', 10)
    ...
    def publish_rpy(self, roll_deg, pitch_deg, yaw_deg):
        ...
        # Manual RPY -> quaternion (same as tf2::Quaternion::setRPY)
        ...
        self.get_logger().info(
            ...
            f'q: [w={msg.w:.4f}, x={msg.x:.4f}, y={msg.y:.4f}, z={msg.z:.4f}]'
        )
...
def main():
    ...
    for i, axis in enumerate(['Roll', 'Pitch', 'Yaw']):
        ...
        slider = tk.Scale(root, from_=-180, to=180, resolution=1,
                          orient=tk.HORIZONTAL, length=300, variable=var,
                          command=lambda _,: None)  # update handled on publish
